...
h2b_test <- h2b %>%
  mutate(season = case_when(
    ...
    str_sub(decision_date,1,1) == "6/" ~ "summer",
    str_sub(decision_date,1,1) == "7/" ~ "summer",
    str_sub(decision_date,1,1) == "8/" ~ "summer",
    str_sub(decision_date,1,1) == "9/" ~ "fall",
    str_sub(decision_date,1,2) == "10/" ~ "fall",
    str_sub(decision_date,1,2) == "11/" ~ "fall",
    str_sub(decision_date,1,2) == "12/" ~ "winter"
  ))
  
h2b_test %>%
  filter(season == "spring") %>%
  select(decision_date, season)
...
#   full_join(h2b_2) 
#   
# h2b_filter <- h2b_filter_1 %>%
#   full_join(h2b_filter_2)
# 
# h2b <- h2b %>%
#   full_join(h2b_filter)
# 
# h2b <- h2b %>%
...
test_fold <- 1
test <- h2b_cv %>% filter(fold == test_fold)
train <- h2b_cv %>% anti_join(test, by = "id")
mod <- lm(prop_workers_certified ~ occupation_category + prevailing_wage + season + region + nbur_workers_requested, data = train)
