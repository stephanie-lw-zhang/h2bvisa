...
>>>>>>> 6b3ca37747a0723df561d971d364fc3df909348b
runtime: shiny
---
...
h2b <- h2b %>%
  mutate(time = paste(year, submitted_season, sep = " ", collapse = NULL))
```
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      selectInput(inputId = "y", 
                  label = "Y-axis:",
                  choices = c("Wage" = "basic_rate_of_pay", 
                              "Number of Workers Requested" = "nbur_workers_requested", 
                              "Number of Workers Certified" = "nbr_workers_certified"),
                  selected = "basic_rate_of_pay"),
      
      # Select variable for x-axis
      selectInput(inputId = "x", 
                  label = "X-axis:",
                  choices = c("Time" = "time", 
                              "Wage" = "basic_rate_of_pay", 
                              "Number of Workers Requested" = "nbur_workers_requested", 
                              "Number of Workers Certified" = "nbr_workers_certified"), 
                  selected = "time"),
      ...
      selectInput(inputId = "z", 
                  label = "Color by:",
                  choices = c("Job Category" = "occupation_category", 
                              "Season (submitted)" = "submitted_season", 
                              "Year" = "year", 
                              "Region" = "region"),
                  selected = "occupation_category"),
...
server <- function(input, output) {
  ...
  output$scatterplot <- renderPlot({
    ggplot(data = visa_sample(), aes_string(x = input$x, y = input$y,
                                              color = input$z)) +
...
#     ggplot(mapping = aes(x = input$category, y = count(input$category)) +
